...
{
  //        获取车辆参数
  if (!controller_nh.getParam("wheel_radius", wheel_radius_) || !controller_nh.getParam("track", track_) ||
      !controller_nh.getParam("wheelbase", wheelbase_) || !controller_nh.getParam("frame", frame_) ||
      !controller_nh.getParam("max_power", max_power_) || !controller_nh.getParam("k_1", k_1_) ||
...
void HeroChassisController::update(const ros::Time& time, const ros::Duration& period)
{
  ...
  //        使用正向运动学计算小车当前的整体速度
  double now_v_x =
      wheel_radius_ * (now_front_left_vel_ + now_front_right_vel_ + now_back_left_vel_ + now_back_right_vel_) / 4;
  double now_v_y =
      wheel_radius_ * (-now_front_left_vel_ + now_front_right_vel_ + now_back_left_vel_ - now_back_right_vel_) / 4;
  ...
  double sum_w2 = pow(now_back_left_vel_, 2) + pow(now_front_left_vel_, 2) + pow(now_back_right_vel_, 2) +
                  pow(now_front_right_vel_, 2);

  double power_in = sum_wt + k_1_ * sum_t2 + k_2_ * sum_w2;
  double k = 1;
  if (power_in > max_power_)
  {
    k = (-sum_wt + sqrt(sum_w2t2 - 4 * k_1_ * sum_t2 * (k_2_ * sum_w2 - max_power_))) / (2 * k_1_ * sum_t2);
    ...
    ROS_INFO("cmd_w of wheel is %f %f %f %f, k is %f", back_left_cmd, front_left_cmd, back_right_cmd, front_right_cmd,
             k);
  }
