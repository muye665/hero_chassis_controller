...
int main(int argc, char** argv)
{
  ...
  if (getparam)
  {
    if (frame == "odom")
    {
      ...
      ROS_INFO(" W: forward, S: backward, A: left, D: right, Q: rotate left, E: rotate right, X: stop.(Q/E cannot use "
               "in Rotation mode)");
      ROS_INFO("-----------------------------------------------------");
    }
  ...
  while (ros::ok())
  {
    ...
    // 打印当前速度值
    ROS_INFO("linear.x: %.2f, linear.y: %.2f, angular.z: %.2f", twist.linear.x, twist.linear.y, twist.angular.z);
    // 按照循环频率延时
    ros::spinOnce();
    loop_rate.sleep();
  }
